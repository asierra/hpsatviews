...
- **File logging is implemented and unreachable, and that is not an oversight left undocumented.** `logger_log()` honours `log_to_file`/`log_file` and writes every message to the file with an `fflush`, but `logger_init()` — the only initializer anything calls (`src/main.c`) — sets just `min_level` and `use_colors`. The four fields that would turn it on (`log_to_file`, `log_filename`, `include_timestamp`, `include_location`) never leave their static initializers, the only function that sets `log_to_file = true` is `logger_set_file()`, which has no callers, and there is no `--log-file` flag. So the feature works and nothing can switch it on. Related: `logger_cleanup()` is invoked only from `logger_log()`'s `LOG_FATAL` branch, so on a normal exit the file would never be closed. Decide it rather than inherit it: either expose `--log-file <path>` (three lines in `main.c`, plus closing the file on normal exit) or delete file output along with those four fields and the three unused setters. The dead-code sweep of 1.2.0 deliberately left them: unlike the seven functions it did remove, these are a coherent module API, and one of them is the only thing standing between the tool and working file logs.
