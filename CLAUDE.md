...
- **`docs/hpsatviews.schema.json` now describes what the emitter actually writes, and `tests/test_json.sh` validates it on every run** (realigned 2026-09-12, schema title v1.1). The drift it used to carry — `timestamp_iso` vs the emitted `timestamp`, a `composite` value in the `command` enum, `geometry`/`channels` both required when a plain `gray` run emits no geometry at all — is closed, and the direction of authority is fixed: **the schema follows `src/metadata.c`, not the other way round**. If you add a key to the sidecar, add it to the schema in the same commit or the suite fails. The validation step is a hard dependency (`python3-jsonschema`), deliberately not a silent skip.
