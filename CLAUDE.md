...
**Day/night blending limits are in `include/daynight_mask.h`** (`HPSV_DN_TERMINATOR` 88°, `HPSV_DN_PENUMBRA` 3°), shared with `src/cuda/daynite_cuda.cu` through `cuda_daynite.h`. They are satpy's `DayNightCompositor` defaults (`lim_low=85`, `lim_high=88`); the mask interpolates on `sin(elevation)`, which is identically `cos(SZA)`, the same variable satpy uses, so aligning the two is just the constants. `HPSV_DN_TERMINATOR` must not exceed `HPSV_SUNZ_LIMIT`, or the blend pulls in a day side that `apply_solar_zenith_correction()` has already zeroed, and a seam appears. These were 85°/10° (blend 75–85) until v1.2.0, which put a grey wash of the nocturnal composite over a wide band of broad daylight.
