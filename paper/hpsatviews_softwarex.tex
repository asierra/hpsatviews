...
\url{https://doi.org/10.5281/zenodo.21893553} (archived v1.1.0; development
repository at \url{https://github.com/asierra/hpsatviews}) \\
C3 & Permanent link to Reproducible Capsule &
\url{https://doi.org/10.5281/zenodo.21893553} --- the archive carries the
\texttt{reproduction/} scripts that fetch the public NOAA sample data and
re-run the benchmarks of Section~\ref{sec:performance}, the regression suite
under \texttt{tests/}, and the figure sources under \texttt{paper/}. \\
...
\texttt{libpng}, \texttt{libgdal}. Optional GPU path: an
...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=0.60\textwidth]{processing_workflow.png}
    ...
    stages for Rayleigh atmospheric correction, CLAHE contrast enhancement,
    and geographic reprojection before writing the final image output and a
    JSON metadata sidecar. Shaded stages are optional and are shown with the
    flag that enables them.}
    \label{fig:workflow}
\end{figure}
...
The software reached its first public release (v1.0.0) in June 2026 and its
current release (v1.1.0), the one measured throughout this paper, in August
2026; every release is archived on Zenodo, with
\href{https://doi.org/10.5281/zenodo.20817973}{10.5281/zenodo.20817973} as the
concept DOI resolving to the latest version. The optional GPU backend now
accelerates the
