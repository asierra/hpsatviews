...
%% Performance placeholders — fill from reproduction/bench_server.sh on the A30
%% server (Xeon Gold 6326 CPU vs. A30 GPU, same machine). \perfTBD renders a
%% loud marker so no cell is missed before submission.
\newcommand{\PRODSERVER}{an NVIDIA A30 GPU (Intel Xeon Gold 6326)}
\newcommand{\perfTBD}{\fbox{\textbf{TBD}}}
...
once per scene. The CPU path remains the bit-for-bit correctness reference and
the fallback for any mode or option without a GPU kernel. Independently of the
...
\begin{itemize}
    ...
    keeping the data resident on the device across the pipeline. Output is
    validated identical to the CPU reference; unsupported modes fall back to the
    CPU transparently.
\end{itemize}
...
per-scene wall time on full-disk scenes. Reading and decompressing the input
NetCDF in parallel with \texttt{libdeflate} reduces variable decompression by
roughly $9\times$ over the single-threaded HDF5 path, and writing a fast tiled
GeoTIFF instead of a full Cloud-Optimized pyramid removes the dominant share of
the output cost --- both benefit the CPU-only build. With the CUDA backend, the
compute-heavy stages are accelerated further, as summarized in
Table~\ref{tab:perf}; the CPU/OpenMP path remains the reference and produces
identical output.

\begin{table}[!ht]
\centering
\caption{Full-disk (10{,}848$\times$10{,}848) true-color with Rayleigh
correction on \PRODSERVER: CPU/OpenMP build versus the optional CUDA build.
Output is validated identical between the two paths.}
\label{tab:perf}
\begin{tabular}{@{}lrr@{}}
\toprule
\textbf{Stage} & \textbf{CPU (OpenMP)} & \textbf{CUDA} \\
\midrule
Viewing geometry (solar + satellite) & \perfTBD & \perfTBD \\
Rayleigh LUT correction (per channel) & \perfTBD & \perfTBD \\
Geometric reprojection to lat/lon & \perfTBD & \perfTBD \\
\midrule
End-to-end wall time & \perfTBD & \perfTBD \\
\bottomrule
\end{tabular}
\end{table}
