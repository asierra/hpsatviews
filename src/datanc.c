...
void dataf_apply_gamma(DataF *data, float gamma, float min_val, float max_val) {
    ...
    if (fabsf(gamma - 1.0f) < 1e-6)
        return;

    float range = max_val - min_val;

    // Skip normalization if the range is degenerate.
    if (range <= 0.0f || IS_NONDATA(min_val))
        return;
...
#pragma omp parallel for
    for (unsigned int by = 0; by < bh; by++) {
        for (unsigned int bx = 0; bx < bw; bx++) {
            ...
            float sum = 0.0f;
            int count = 0;
            float vals[4] = {input->data_in[y0 * w + x0], input->data_in[y0 * w + x1],
                             input->data_in[y1 * w + x0], input->data_in[y1 * w + x1]};
            for (int k = 0; k < 4; k++) {
